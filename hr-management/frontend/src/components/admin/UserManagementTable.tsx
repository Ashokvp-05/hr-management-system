...
import { Users, Search, Settings, ShieldAlert, Plus, FileText, Loader2, MoreHorizontal, ShieldCheck, UserX, Key, Trash2 } from "lucide-react"
import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuItem,
        ...
        <Card className="border-indigo-100 dark:border-indigo-900/50 bg-white dark:bg-slate-900 shadow-md">
            <CardHeader className="border-b border-indigo-100 dark:border-indigo-900/50 bg-indigo-50/50 dark:bg-indigo-900/10 py-4 px-6 flex flex-row items-center justify-between">
                ...
                <div className="flex gap-2">
                    <Button size="sm" variant="outline" className="bg-white font-bold text-xs pointer-events-none opacity-50">
                ...
                {loading ? (
                    ...
                    <div className="divide-y divide-slate-100 dark:divide-slate-800 h-[500px] overflow-y-auto">
                        {filteredUsers.map((u, i) => (
                            <div key={u.id} className="grid grid-cols-12 gap-4 p-4 items-center hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                                <div className="col-span-1 text-center font-mono text-[10px] text-slate-300">
                                    {i + 1}
                                </div>
